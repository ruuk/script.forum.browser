...
class FileLock(object):
	...
	def __init__(self, protected_file_path, timeout=None, delay=1, lock_file_contents=None):
		...
		self._check_stale_lock = False
		if self._lock_file_contents is None:
			self._lock_file_contents = str(time.time())
			self._check_stale_lock = True
			#for arg in sys.argv:
	...
	def acquire(self, blocking=True):
		...
		"""
		if self.lockIsStale(): self.purge()
	...
	def lockIsStale(self):
		if not self.timeout: return False
		if not os.path.exists(self.lockfile): return False
		try:
		...
		try:
			...
			return True
		if time.time() - last > self.timeout: return True
		return False
