...
class ScraperForumBrowser(forumbrowser.ForumBrowser):
	...
	def readURL(self,url,callback=None,force_login=False,is_html=True):
		if not url:
			...
			return ''
		if not callback: callback = self.fakeCallback
		if self.formats.get('login_required') == 'True' or force_login or (self.alwaysLogin and self.password):
...
#		#xbmcgui.unlock()
	...
	def doMenu(self):
		...
		if FB.canSubscribeThread(self.tid):
			subscribe = len(options)
			options.append(__language__(30236))
		idx = xbmcgui.Dialog().select(__language__(30051),options)
		if idx == 0:
			self.stopThread()
			...
			result = FB.subscribeThread(self.tid)
			if result == True:
...
class ThreadsWindow(PageWindow):
	...
	def openRepliesWindow(self):
		...
		if item.getProperty('is_forum') == 'True':
			...
			w = RepliesWindow("script-forumbrowser-replies.xml" , __addon__.getAddonInfo('path'),THEME,tid=tid,fid=fid,lastid=lastid,topic=topic,reply_count=reply_count,parent=self)
			w.doModal()
			del w
...
def addForumToOnlineDatabase(name,url,desc,logo,ftype,dialog=None):
	...
	if msg == 'OK':
		...
		xbmcgui.Dialog().ok('Not Added','Forum not added:',str(msg))
