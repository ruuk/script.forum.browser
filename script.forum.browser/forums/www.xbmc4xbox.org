...
filter:threads=<tr>.*?href="\./viewtopic.php?.*?t=(?P<threadid>\d+).*?class="topictitle">(?P<title>.*?)</a>.*?<a href="./memberlist.php.*?>(?P<starter>.*?)</a>.*?<a href="./memberlist.php.*?>(?P<lastposter>.*?)</a>.*?<a href="\./viewtopic.php\?.*?;p=(?P<lastid>\d+).*?</tr>
...
filter:post=<a name="p(?P<postid>\d+)"></a>.*?<b class="postauthor".*?>(?P<user>.*?)</b>.*?Post subject:</b>(?P<title>.*?)</div>.*?(?P<date>\w+\s\w+\s\d+,\s\d+\s\d+:\d+\s\w\w).*?(?:(?:<img src="(?P<avatar>\./download/file.php\?avatar=.*?)")|(?:,)).*?<div class="postbody" id="postdiv.*?>(?P<message>.*?)<br clear="all" /><br />.*?\?mode=viewprofile&amp;u=
