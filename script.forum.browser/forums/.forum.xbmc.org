...
#<img src="http://forum.xbmc.org/images/xbmc/xbmc-lettering.png"
...
filter:quote_start=(?is)<div class="smallfont" style="margin-bottom:2px">Quote:</div><table.*?><tr><td.*?(?:inset"><div>Originally Posted by <strong>(?P<user>.+?)</strong>(?:</div><div style="font-style:italic)?)?(?:<a href="showthread.php\?[^>]*?p=(?P<postid>\d+)#\w+\d+"[^>]*?)?">
filter:quote_end=</table>
